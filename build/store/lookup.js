...
function nextProxy(store) {
    ...
    logger_1.logger.info(`ℹ [${store.name}] Next proxy index: ${store.currentProxyIndex} / Count: ${store.proxyList.length}`);
...
async function lookupCard(browser, store, page, link) {
    ...
    if (await lookupCardInStock(store, page, link)) {
        const givenUrl = link.cartUrl && config_1.config.store.autoAddToCart
            ? link.cartUrl
            : link.url;
